...
app.use(cors({
  origin: (origin, callback) => {
    ...
    if (!origin || process.env.NODE_ENV === 'development' || allowed.includes(origin)) {
      ...
      console.warn(`[CORS] Allowed origins are: ${JSON.stringify(allowed)}`);
...
// Enterprise limit for LLM endpoints (Financial Abuse Protection)
const aiLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, // 1 hour
  max: 20, // Strict limit per IP
  message: { error: 'LLM AI Generation Rate limit exceeded', message: 'You have reached your hourly AI styling limit. Please try again later.' },
});
app.use('/api/outfits/suggestion', aiLimiter);
app.use('/api/fit-check', aiLimiter);
